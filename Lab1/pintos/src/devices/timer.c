...
timer_sleep (int64_t ticks) 
{
  ...
  /*
  thread yeild puts thread off the cpu but it is not alseep.
  */
...
void wakeup_helper(struct thread *t, void *aux) {
	if(t->wakeup_time == timer_ticks()) thread_unblock(t);
}
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ticks++;
  thread_tick ();
  thread_foreach(wakeup_helper, NULL);
  // printf("%s\n", "timer interrupt");
  // struct list_elem *begin = list_begin(&sleeping_list);
  // struct list_elem *end = list_end(&sleeping_list);
  // struct list_elem *next_elem;
  // struct thread *curr_thread;
  // while(begin != end) {
  // 	printf("%s\n", "iterating through sleeping list");
  //   curr_thread = list_entry(begin, struct thread, sleep_elem);
  //   printf("%s\n", "got current thread");
  //   if(curr_thread->wakeup_time >= timer_ticks()) {
  //     next_elem = list_remove(begin);
  //     curr_thread = list_entry(begin, struct thread, sleep_elem);
  ...
  // 	if(f->wakeup_time >= timer_ticks()) {
  // 		struct list_elem *e_ = list_remove(e);
  // 		struct thread *f_ = list_entry(e, struct thread, sleep_elem);
  // 		thread_unblock(f);
  // 	}
  // }
}
